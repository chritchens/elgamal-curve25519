...
#[derive(Copy, Clone, Eq, PartialEq, Debug)]
pub struct CypherText(CompressedRistretto);

impl CypherText {
    /// `from_point` creates a new `CypherText` from a `CompressedRistretto`.
    pub fn from_point(point: CompressedRistretto) -> CypherText {
        CypherText(point)
    }

    /// `to_point` returns the inner `CompressedRistretto` of the `CypherText`.
    pub fn to_point(&self) -> CompressedRistretto {
        self.0
    }

    /// `from_hash` creates a new `CypherText` from a 64 bytes hash.
    pub fn from_hash<D>(digest: D) -> CypherText
        where D: Digest<OutputSize = U64> + Default
    {
        let point = RistrettoPoint::from_hash(digest);
        CypherText(point.compress())
    }

    /// `from_slice` creates a new `CypherText` from a slice of bytes.
    pub fn from_slice(buf: [u8; 32]) -> CypherText {
        let point = CompressedRistretto::from_slice(&buf[..]);
        CypherText(point)
    }
...
pub fn encrypt(msg: Message, pk: PublicKey, sk: PrivateKey) -> Result<CypherText, String> {
    ...
    if let Some(msg_point) = msg.to_point().decompress() {
        if let Some(shared_point) = shared(pk, sk)?.decompress() {
            let cyph_decomp = msg_point + shared_point;

            let cyph = CypherText(cyph_decomp.compress());
            Ok(cyph)
        } else {
...
pub fn decrypt(cyph: CypherText, pk: PublicKey, sk: PrivateKey) -> Result<Message, String> {
    if sk.to_public().to_point().ct_eq(&pk.to_point()).unwrap_u8() == 1u8 {
        return Err("same private keys".into());
    }

    if let Some(cyph_point) = cyph.to_point().decompress() {
        if let Some(inv_shared_point) = inverse_shared(pk, sk)?.decompress() {
            let msg_point = cyph_point + inv_shared_point;
...
fn test_encryption() {
    for _ in 0..10 {
        ...
        let msg2 = decrypt(cyph, pk1, sk2).unwrap();
